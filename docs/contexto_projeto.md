...
**Última atualização:** fim da Etapa 4
...
**Decisões firmadas:** sem LangChain (avaliado e descartado); sem OpenAI API; sem `sentence-transformers` (incompatível com o deploy).
...
| 5 | Classificação e priorização de leads | ⬜ **próxima** |
...
| Classificação/priorização de leads | ⬜ Etapa 5 |
...
├── docs/
│   ├── decisoes_tecnicas.md        # decisões, 23 bugs, 26 limitações, métricas
...
**Pastas previstas e ainda vazias:** `src/scoring/`, `src/followup/`, `src/reporting/`, `src/observability/`
...
ORQUESTRADOR  ← única porta de entrada do domínio
  persiste entrada → qualifica → recomenda → gera resposta → persiste saída
         ↓                              ↓
...
CAMADA DE IA                     SQLite (7 tabelas)
  ...
  demo_engine
         ↓
RECOMENDAÇÃO
  retriever (TF-IDF) · ranker (filtro + score)
```
...
## 9. Etapa 5 — próxima (não iniciada)

**Objetivo:** atribuir score (0–100) e temperatura (quente/morno/frio) a cada lead, para priorização pelo corretor.

**Arquivos previstos:**

- `src/scoring/rules.py` — regras de negócio explicáveis
- `src/scoring/classifier.py` — `scikit-learn`, se justificável

**Decisão pendente:** regras explícitas, modelo de ML, ou híbrido. Considerar que não há dados históricos reais de conversão — um classificador treinado exigiria dados sintéticos rotulados, o que precisa de justificativa.

**Sinais candidatos para o score:** completude da qualificação, urgência declarada, disponibilidade informada, faixa de orçamento, engajamento (número de turnos), intenção identificada.

**Integração:** o campo `score` e `temperature` já existem no modelo `Lead` e na tabela; `LeadRepository.listar()` já ordena por score decrescente.

---

## 10. Etapas 6 a 8 — planejadas

**Etapa 6 — Agendamento, follow-up e resumo**
...
3. Informar a etapa desejada — provavelmente **Etapa 5**
